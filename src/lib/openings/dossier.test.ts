...
describe("openingDossier", () => {
  ...
  it("labels system openings", () => {
    ...
    for (const id of ["alapin", "english", "queens-gambit"]) {
      const opening = openings.find((o) => o.id === id);
      assert.ok(opening, id);
      assert.match(openingDossier(opening), /^System · White/, id);
    }
    assert.match(
      openingDossier(openings.find((o) => o.id === "caro-kann")!),
      /^System · Black/,
    );
    assert.match(
      openingDossier(openings.find((o) => o.id === "slav")!),
      /^System · Black/,
    );
  });
});
...
describe("weapon marks", () => {
  it("covers every production opening including the five new systems", () => {
